...
def FirstPartDipath (γ : Dipath x₀ x₁) (T : I) : Dipath x₀ (γ T) := {
  FirstPart (γ : Path x₀ x₁) T with
  dipath_toPath := first_part_is_dipath γ.dipath_toPath T
}

def SecondPartDipath (γ : Dipath x₀ x₁) (T : I) : Dipath (γ T) x₁ := {
...
lemma first_part_apply (γ : Dipath x₀ x₁) (T t : I) :
  (FirstPartDipath γ T) t = γ ⟨ T* t, unitInterval.mul_mem T.2 t.2⟩ := rfl

@[simp]
lemma second_part_apply (γ : Dipath x₀ x₁) (T t : I) :
  (SecondPartDipath γ T) t = γ ⟨(σ T : ℝ) * (t : ℝ) + (T : ℝ), interp_left_mem_I T t⟩ := rfl
...
lemma first_trans_second_reparam_eq_self (γ : Dipath x₀ x₁) {T: I} (hT₀ : 0 < T) (hT₁ : T < 1) :
  γ = ((FirstPartDipath γ T).trans (SecondPartDipath γ T)).reparam
